...
/** Read a stored object fully into bytes (robust across stream types). */
export async function getObjectBytes(key: string): Promise<Uint8Array> {
  const object = await getPrivateObject(key);
  const buffer = await new Response(object.body as BodyInit).arrayBuffer();
  return new Uint8Array(buffer);
}

/**
 * Server-side upload of generated bytes for a letter (e.g. a signed PDF).
 * Returns the full (prefixed) object key.
 */
export async function putLetterObject(
  workspaceId: string,
  letterId: string,
  filename: string,
  contentType: string,
